...
from backend.app.crud.projects import list_all_projects, list_project_by_id, create_project, update_project, delete_project
...
def update_project(project_id: int, project_data: schemas.ProjectUpdate, db: Session = Depends(get_db), user: models.User = Depends(get_current_user)):
    try:
