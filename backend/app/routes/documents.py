...
from backend.app.crud.documents import list_all_docs, list_doc_by_id, create_doc, update_doc, delete_doc
...
def get_docs(db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
...
def get_doc(doc_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
...
def update_doc(doc_id: int, doc_data: schemas.ProjectUpdate, db: Session = Depends(get_db), user: models.User = Depends(get_current_user)):
    try:
...
def delete_doc(doc_id: int, db: Session = Depends(get_db), user: models.User = Depends(get_current_user)):
    try:
